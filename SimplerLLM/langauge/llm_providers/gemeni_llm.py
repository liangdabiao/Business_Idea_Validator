...
load_dotenv()
os.environ["GEMENI_API_KEY"] = "false"
# Constants
...
def generate_text(user_prompt, model, max_tokens=12000, top_p=1.0, temperature=0.7):
    if not user_prompt or not isinstance(user_prompt, str):
        raise ValueError("user_prompt must be a non-empty string.")
...
def generate_full_response(user_prompt, model, max_tokens=12000, top_p=1.0, temperature=0.7):
    if not user_prompt or not isinstance(user_prompt, str):
        raise ValueError("user_prompt must be a non-empty string.")
